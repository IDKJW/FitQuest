...
def generate_random_exercise():
    global exercises_shown, progress_var
    ...
    if len(exercises_shown) == len(exercises):
        ...
        generate_button.config(text="Well Done")  # Change the button text
        return
    ...
    progress_var.set(len(exercises_shown) / len(exercises) * 100)

# Restarts the exercise routine when the button is clicked


def restart_exercise():
    global exercises_shown
    exercises_shown = []
    generate_button.config(text="Generate Exercise")  # Reset the button text
