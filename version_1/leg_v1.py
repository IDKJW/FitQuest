...
cursor.execute("SELECT exercise, reps, image FROM Leg")
...
leg = tk.Tk()
leg.title("leg Exercise")
leg.geometry("500x800")
leg.resizable(width=False, height=False)

# Create background image
background_image = PhotoImage(file="assets/Aim high.png")
background_label = Label(leg, image=background_image)
...
exercise_label = tk.Label(leg, text="Exercise:", background="black", fg="white")
exercise_label.pack()

reps_label = tk.Label(leg, text="Reps:", background="black", fg="white")
reps_label.pack()

# Create an empty label for the image
image_label = tk.Label(leg, background="black")
image_label.pack()

# Create a button to generate random exercise
generate_button = tk.Button(leg, text="Generate Exercise", command=generate_random_exercise, background="black", fg="black")
generate_button.pack()

# Create a label with clickable text
leg_text_button = tk.Label(leg, text="<- leg", fg="blue", cursor="hand2", background="black")
leg_text_button.pack()

# Bind the click event to the label
leg_text_button.bind("<Button-1>", home_button)
