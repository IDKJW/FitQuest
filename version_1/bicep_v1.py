...
cursor.execute("SELECT exercise, reps, image FROM Biceps")
...
bicep = tk.Tk()
bicep.title("bicep Exercise")
bicep.geometry("500x800")
bicep.resizable(width=False, height=False)

# Create background image
background_image = PhotoImage(file="assets/Aim high.png")
background_label = Label(bicep, image=background_image)
...
exercise_label = tk.Label(bicep, text="Exercise:", background="black", fg="white")
exercise_label.pack()

reps_label = tk.Label(bicep, text="Reps:", background="black", fg="white")
reps_label.pack()

# Create an empty label for the image
image_label = tk.Label(bicep, background="black")
image_label.pack()

# Create a button to generate random exercise
generate_button = tk.Button(bicep, text="Generate Exercise", command=generate_random_exercise, background="black", fg="black")
generate_button.pack()

# Create a label with clickable text
bicep_text_button = tk.Label(bicep, text="<- Back", fg="blue", cursor="hand2", background="black")
bicep_text_button.pack()

# Bind the click event to the label
bicep_text_button.bind("<Button-1>", text_clicked)
