...
cursor.execute("SELECT exercise, reps, image FROM Chest")
...
chest = tk.Tk()
chest.title("chest Exercise")
chest.geometry("500x800")
chest.resizable(width=False, height=False)

# Create background image
background_image = PhotoImage(file="assets/Aim high.png")
background_label = Label(chest, image=background_image)
...
exercise_label = tk.Label(chest, text="Exercise:", background="black", fg="white")
exercise_label.pack()

reps_label = tk.Label(chest, text="Reps:", background="black", fg="white")
reps_label.pack()

# Create an empty label for the image
image_label = tk.Label(chest, background="black")
image_label.pack()

# Create a button to generate random exercise
generate_button = tk.Button(chest, text="Generate Exercise", command=generate_random_exercise, background="black", fg="black")
generate_button.pack()

# Create a label with clickable text
chest_text_button = tk.Label(chest, text="<-Back", fg="blue", cursor="hand2", background="black")
chest_text_button.pack()

# Bind the click event to the label
chest_text_button.bind("<Button-1>", text_clicked)
