...
def home(event):
    triceps.destroy()
    subprocess.Popen([python_executable, "design/version_1/home_1.py"])
    return
...
triceps = tk.Tk()
triceps.title("triceps Exercise")
triceps.geometry("500x800")
triceps.resizable(width=False, height=False)

# Create background image
background_image = PhotoImage(file="assets/Aim high.png")
background_label = Label(triceps, image=background_image)
...
reps_label = tk.Label(triceps, text="Reps:", background="black", fg="white")
reps_label.pack()

# Create an empty label for the image
image_label = tk.Label(triceps, background="black")
image_label.pack()
...
generate_button = tk.Button(
    ...
    command=generate_random_exercise,
    background="black",
    fg="black")
generate_button.pack()

# Create a label with clickable text
triceps_text_button = tk.Label(
    triceps,
    text="<-Back",
    fg="blue",
    cursor="hand2",
    background="black")
triceps_text_button.pack()

# Bind the click event to the label
triceps_text_button.bind("<Button-1>", home)
triceps.mainloop()
